 ...
 * @author Jeff Butler
 */
public class HMyBatisGeneratorConfigurationParser {
    private Properties properties;

    public HMyBatisGeneratorConfigurationParser(Properties properties) {
        super();
        if (properties == null) {
    ...
    private void parseClassPathEntry(Configuration configuration, Node node) {
        Properties attributes = parseAttributes(node);
        ClassPathResource classPathResource = new ClassPathResource(attributes.getProperty("location"));
        if (classPathResource.exists()) {
            try {
                configuration.addClasspathEntry(classPathResource.getFile().getPath());
            } catch (IOException e) {
